...
"""
Except for the functions get_conv_output, forward_after_conv
...
class Enformer(PreTrainedModel):
    ...
    def forward(
        ...
        # Add the following two functions to the original code.
    def get_conv_output(self, x):
            """
            x: One-hot encoded DNA sequence of shape (batch, seq_len, 4)
            return: After passing through the Conv Tower, tensor of shape (batch, seq_len_downsampled, dim_out)
            """
            # 1) (b, n, d) -> (b, d, n)
            x = rearrange(x, 'b n d -> b d n')
    ...
    def forward_after_conv(
        ...
        """
        Given a tensor x (batch, ds_seq_len, conv_dim) that has already passed through the Conv Tower,
        this method performs Transformer -> Crop -> Final Pointwise -> Heads,
        and returns the final result (either as a dictionary or a single head tensor).
        """

        # x shape: (batch, seq_len_downsampled, dim_out)
        # if needed, move to device
        x = x.to(self.device)

        no_batch = (x.ndim == 2)
        if no_batch:
            x = rearrange(x, '... -> () ...')  # (seq_len, dim) -> (1, seq_len, dim)
        if exists(target_length):
            self.set_target_length(target_length)

        # Transformer (with or without checkpointing)
        ...
        if exists(target):
            assert exists(head), "Direct loss computation is not possible without the head parameter."
            if return_corr_coef:
                return pearson_corr_coef(out, target)
            return poisson_loss(out, target)
        if return_embeddings:
