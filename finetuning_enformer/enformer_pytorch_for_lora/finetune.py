...
"""
Except for the classes HeadAdapterWrapper_For_Lymphocyte_Vector_Input
this file is identical to
https://github.com/lucidrains/enformer-pytorch/blob/main/enformer_pytorch/finetune.py
and has been copied from it.
"""
import torch
...
# Add the following three classes to the original code.
class HeadAdapterWrapper_For_Lymphocyte_Vector_Input(nn.Module):
    """
    The header implementation was designed with reference to the evaluation code from the study
    'Personal Transcriptome Variation Is Poorly Explained by Current Genomic Deep Learning Models' (PMCID: PMC10703684).
    """
    def __init__(
            self,
            *,
        ...
        super().__init__()
        # assert isinstance(enformer1, Enformer)
        # assert isinstance(enformer2, Enformer)
        self.enformer1 = enformer1
        self.enformer2 = enformer2
    ...
    def forward(
        ...
        # seq1, seq2: (batch_size, sequence_length, 4)
        # 5110 = Lymphocyte
        preds1 = self.enformer1.forward_after_conv(tensor1)['human'][:, :, 5110]
        preds2 = self.enformer2.forward_after_conv(tensor2)['human'][:, :, 5110]
        # preds1, preds2: (batch_size, seq_length)

        # Slicing is applied on the sequence dimension for indexing.
        # tss10_expr_1: (batch_size,)
        tss10_expr_1 = torch.mean(preds1[:, (448 - 5):(448 + 5)], dim=1)
        tss10_expr_2 = torch.mean(preds2[:, (448 - 5):(448 + 5)], dim=1)
        average_tss10_expr = (tss10_expr_1 + tss10_expr_2) / 2  # (batch_size,)
        return average_tss10_expr.unsqueeze(1)  # (batch_size, 1)
